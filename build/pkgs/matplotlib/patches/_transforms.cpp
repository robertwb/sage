...
MPL_Point::MPL_Point(LazyValue* x, LazyValue*  y) : _x(x), _y(y) {
  _VERBOSE("MPL_Point::MPL_Point");
  Py_INCREF(x);
  Py_INCREF(y);
}
...
Bbox::Bbox(MPL_Point* ll, MPL_Point* ur) : _ll(ll), _ur(ur), _ignore(1) {
  _VERBOSE("Bbox::Bbox");
...
Bbox::_deepcopy() {
  ...
  return Py::asObject( new Bbox( new MPL_Point(new Value(minx), new Value(miny) ),
				 new MPL_Point(new Value(maxx), new Value(maxy) )));
}
Py::Object
...
_transforms_module::new_point (const Py::Tuple &args)
{
  ...
  return Py::asObject(new MPL_Point(x, y));
...
_transforms_module::new_bbox (const Py::Tuple &args)
{
  ...
  if (!MPL_Point::check(args[0]))
    throw Py::TypeError("MPL_Point(p1,p2) expected a MPL_Point for p1");
  if (!MPL_Point::check(args[1]))
    throw Py::TypeError("MPL_Point(p1,p2) expected a MPL_Point for p2");

  MPL_Point* ll = static_cast<MPL_Point*>(args[0].ptr());
  MPL_Point* ur = static_cast<MPL_Point*>(args[1].ptr());
...
MPL_Point::init_type()
{
  _VERBOSE("MPL_Point::init_type");
  behaviors().name("MPL_Point");
  behaviors().doc("A point x, y");

  add_varargs_method("x",    &MPL_Point::x,     "x()\n");
  add_varargs_method("y",    &MPL_Point::y,     "y()\n");
  add_varargs_method("reference_count", &MPL_Point::reference_count);
}
